...
function setSizes() {
  ...
  $.each(types, function(x, o) {
    $(this)
      .css("width", $(window).width() / types.length)
      .css("font-size", $(this).height() / 3 + "px");
  });
}
...
function showAnalog(both = false) {
  try {
    $("#clock-contents").html(
      '<div id="keithapps-clock" style="position: fixed;">' +
        '<img id="keithapps-clockImg" style="position: absolute; height: 100%;" src="/images/clock-circle2N.png" />' +
        "</div>"
    );
    ...
    clockEl.append(
      '<div id="keithapps-secondParent" style="height: 100%; width: 100%; position: absolute;">' +
        '<img id="keithapps-clockSecond" src="/images/clock-hand2.png" style="position: absolute; height: 50%; top: 5px;" />' +
        "</div>"
    );
    clockEl.append(
      '<div id="keithapps-minuteParent" style="height: 100%; width: 100%; position: absolute; ">' +
        '<img id="keithapps-clockMinute" src="/images/clock-hand2.png" style="position: absolute; height: 50%; padding-top: 10%; top: 5px; z-index: 0;" />' +
        "</div>"
    );
    clockEl.append(
      '<div id="keithapps-hourParent" style="height: 100%; width: 100%; position: absolute; z-index: 100">' +
        '<img id="keithapps-clockHour" src="/images/clock-handRed2.png" style="position: absolute; height: 50%; padding-top: 18%; top: 5px; " />' +
        "</div>"
    );
    ...
    var onSize = function() {
      var size = Math.min($(window).width(), $(window).height() - 50) * 0.85;
      if (both) size /= 1.5;
      clockEl.css("width", size);
      clockEl.css("height", size);
      clockEl.css(
        "top",
        $(window).width() < $(window).height() / 1.5
          ? $(window).height() * 0.2
          : 75
      );
      ...
      try {
        ...
        secondParent.css({
          transform:
            "rotate(" +
            (
              (date.getSeconds() /* Make seconds 'tick' */ +
...
function showDigital(both = false) {
  try {
    if ($("#keithapps-digitalClock").length == 0)
      $("#clock-contents").append(
        '<div id="keithapps-digitalClock" style="position: fixed; width: 100%;color:black;"></div>'
      );
    ...
    window.setInterval(function() {
      ...
      var text =
        ...
        1 +
        ":" +
        padInt(time.getMinutes(), 2) +
        ":" +
        padInt(time.getSeconds(), 2) +
        " " +
...
function resizeDigital() {
  ...
  contents.css(
    "font-size",
    (showDate ? $(window).width() / 16 : $(window).width() / 7) + "px"
  );
