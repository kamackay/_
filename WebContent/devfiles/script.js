$.fn.setupHorizontalScroll = function() {
  this.mousewheel(scrollHorizontal);
};
...
var tooltipOptions = { html: true, animated: "fade", delay: 500 };
...
function convertToGoogleSearch(e) {
  for (
    var o = "https://www.google.com/webhp#q=", t = 0, i = e.length;
    i > t;
    t++
  )
    o +=
...
function writeToLog(e) {
  try {
    var o = $("#keithapps-log");
    void 0 !== o && o.append("<p>" + e + "</p>");
  } catch (t) {}
}
...
function notSupported() {
  try {
    showSnackbar(
      "Sorry, but this is not currently supported. Check back in a few days!"
    );
  } catch (err) {}
}
...
function showWatermark(anim = true) {
  $("body").append(
    '<div onclick="openInNewTab(\'http://keithmackay.com\')" id="watermark" style="position: fixed; top: -100%;left: 200%;width: 200px; opacity: .5; cursor: pointer;"><img style="width: 200px;" src="/images/watermark.png" alt="watermark"/></div>'
  );
  var place = function(time = 10) {
    ...
    if ($(window).height() > $(window).width()) $("#watermark").hide();
  };
  if (anim) place(10000);
  else
    $("#watermark").css({
...
function collapseSettings(opt = "") {
  ...
  if (settingsEl.length) {
    ...
    if (closeButton.length) {
      ...
      settingsEl.append(
        '<div style="display:inline;cursor:pointer;height:100%;margin:10px;" onclick="collapseSettings();storeData(\'settingsCollapse\', 1)" class="closeSettings"><i style="color:red;padding:10px;" class="material-icons">close</i></div>'
      );
    }
    settingsEl.animate(
      {
        left: -2 * settingsEl.width()
      },
      {
        duration: opt === "fast" ? 1 : 500,
        queue: false
      }
    );
    ...
    if (shortcut.length) {
      ...
      $("body").append(
        '<div style="position:fixed;left:2px;bottom:2px;cursor:pointer;" id="settingsShortcut" onclick="storeData(\'settingsCollapse\', 0);setTimeout(function (){expandSettings(false)}, 100);"><i style="font-size:35px;color:inherit;" class="material-icons">settings</i></div>'
      );
...
function expandSettings(force = true) {
  ...
  var settingsEl = $("#settings");
  if (settingsEl.length) {
    settingsEl.css("background-color", "#eee").animate(
      {
        left: 0
      },
      {
...
$(document).ready(function() {
  ...
  var a1 = function() {
    if (leftBar.length)
      $.each(leftBar.find(".option"), function(n, o) {
        var t = $(this);
  ...
  try {
    const options = {
      url: (window.keith && window.keith.main ? "." : "..") + "/backend.get",
      type: "POST",
      data: serialize({ type: "version" }),
      complete: function(data) {
        if (data.responseJSON)
          $("body").append(
            '<span onclick="openInNewTab(\'http://keithmackay.com/about/\')" style="font-size:10px;padding:5px;background-color:rgba(255,255,255,.5);position:fixed;bottom:0px;right:0px;cursor:default;margin:0px;" class="well unselectable version">Version: ' +
              data.responseJSON +
              "</span>"
          );
      }
...
function usingMaterialLoading() {
  var materialFunction = function() {
    var materialLoaders = $("body").find(
      ".materialLoad:not(.materialLoad-Handled)"
    );
    if (materialLoaders.length) {
      $.each(materialLoaders, function(n, o) {
        var t = $(this);
        t.append(
      ...
      setTimeout(materialFunction, 50);
    }
  };
  materialFunction();
}
...
function stripComments(str) {
  return str.replace(R_BLOCKS, function(match, qblock, mlcomm) {
    return qblock
      ? match // quoted string, divisor, regex: return as-is
      : mlcomm
      ? " " // multiline comment: replace with space
...
function loadJSAsync(url, id) {
  try {
    $.get(url, function(data) {
      $("head").append(
        '<script id="' + id + '" type="text/javascript">' + data + "</script>"
      );
    }).fail(function() {
  ...
  try {
    ...
    return (
      Object.keys(a).forEach(function(c) {
        b.length > 0 && (b += "&"), (b += c + "=" + encodeURIComponent(a[c]));
      }),
      b
    );
  } catch (a) {
    log("Error In Serialization");
    return showError(a), "CouldNotSerialize";
  }
}
...
function popupForm(contents) {
  ...
  var $win = $(window);
  $("body").append(
    '<div class="well" style="position:fixed;top:' +
      (-2 * $win.height()).toString() +
      'px;left:20vw;height:60vh;width:60vw;text-align:center;" id="popupFormDiv"><i onclick="$(\'#popupFormDiv\').remove()" style="width:100%;text-align:right;" class="material-icons">clear</i></div>'
  );
...
function getDomain(url = document.location) {
