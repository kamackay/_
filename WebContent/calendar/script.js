...
function init() {
  ...
  var todayEl = $("#today");
  var text =
    getMonthName(displayDate) + ", " + displayDate.getFullYear().toString();
  if (settings.anim) {
  ...
  switch (mode) {
    case Modes.month:
      ...
      while (count++ < 7)
        monthEl.append(
          '<div class="month-head" style="cursor:default;"></div>'
        );
      count = 0;
      while (count++ < 42)
        monthEl.append('<div class="month-day">&nbsp;</div>');
      ...
      $.get(reqUrl, function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (dayOfHoliday.find(".holiday").length == 0) {
            //Only allow one holiday per date, for now
            dayOfHoliday.append(
              '<div class="holiday">&nbsp;<br><a href="#" onclick="' +
                (holiday.link == undefined
                  ? ""
                  : "openInNewTab('" + holiday.link + "')") +
                '">' +
                holiday.name +
                "</a></div"
            );
...
$(document).ready(function() {
  ...
  loadBackgroundAsync("/images/material_white.png");
});
